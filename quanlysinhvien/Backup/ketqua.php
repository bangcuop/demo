...
if(isset($_POST['update'])){
	if(isset($_POST['check'])){
		...
		if($num_arr>0){
			for($j=1;$j<=$num_arr;$j++){
				$sql_u=mysql_query('UPDATE ketqua SET p_check=1 WHERE p_id='.$arr_check[$j-1]);
				if($sql_u){
...
if(isset($_POST['delete'])){
	if(isset($_POST['check'])){
		...
		if($num_arr>0){
			for($j=1;$j<=$num_arr;$j++){
				$sql_u=mysql_query('UPDATE ketqua SET p_del=1 WHERE p_id='.$arr_check[$j-1]);
				if($sql_u){
...
if(isset($_POST['undel'])){
				...
				$sql_u=mysql_query('UPDATE ketqua SET p_del=0,p_check=0');
				if($sql_u){
...
if(isset($_POST['new'])){
header("location: them_ketqua.php");
}


$sql=mysql_query("select * from ketqua");
...
<h3> QUẢN LÝ - HIỂN THỊ KẾT QUẢ</h3>
